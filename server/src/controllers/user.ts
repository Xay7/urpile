...
export const register: RequestHandler = async (req, res) => {
  try {
    ...
    const { rows } = await db.query('SELECT email FROM users WHERE email = $1', [email]).catch(error(500, 'Database error'));
    ...
    await db.query('INSERT INTO users(username,password,email) VALUES ($1,$2,$3)', [username, hashedPassword, email]).catch(error(500, 'Database error'));
...
export const login: RequestHandler = async (req, res) => {
  try {
    ...
    const { rows } = await db.query('SELECT * FROM users WHERE email = $1', [email]).catch(error(500, 'Database error'));
    if (!rows.length) {
      return res.status(401).send("Email doesn't exists");
    }

    const validPassword = await bcrypt.compare(password, rows[0].password).catch(error(500, 'Error occured when validating password'));
